...
static const unsigned char lameName[]         PROGMEM = "HAL 1284";
...
drawrow: {
    ...
    if ( x0 < 0 || x0 > TV.hres() || x1 < 0 || x1 > TV.hres() || x1 <= x0 || col > 2)
      goto qwhat;

    TV.draw_row(line, x0, x1, col);
    goto run_next_statement;
  }
...
drawcol: {
    ...
    if ( y0 < 0 || y0 > TV.vres() || y1 < 0 || y1 > TV.vres() || y1 <= y0 || col > 2)
      goto qwhat;

    TV.draw_column(line, y0, y1, col);
    goto run_next_statement;
  }
...
static int inchar()
{
  ...
  switch ( inStream ) {
    ...
    default:
      ...
      {
        if (Serial.available()) {
          /*char inputSerial = Serial.read();
            if ((int)inputSerial == -128) {
            //UP
            } else if ((int)inputSerial == -127) {
            //DOWN
            } else if ((int)inputSerial == -126) {
            //LEFT
            } else if ((int)inputSerial == -125) {
            //RIGHT
            } else {
            return inputSerial;
            }*/
